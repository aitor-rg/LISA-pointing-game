...
Rstep = 0;
Rsig = 0; % 1E-7 also works
Rreg = 0; % 1E-7 also works
gstep = 1;
gsig = 1;
greg = 1;
...
%     x3 = X(13:18,k-1); %s3
    ...
    % Min
    DJsmooth = 3*gamma*((J(1,k)))*W(1:3)/sigma(k-1-LL,Rsig,b,gsig)^2;
    M(1:3,k) = beta*M(1:3,k-1) + DJsmooth*(1-beta);
    U(1:3,k) = U(1:3,k-1) - stepsize(k-1-LL,Rstep,a,gstep)*sigma(k-1-LL,Rsig,b,gsig)^2*(M(1:3,k) + reggain(k-1-LL,Rreg,b,greg)*U(1:3,k-1) );

    DJsmooth = 3*gamma*(J(2,k))*W(7:9)/sigma(k-1-LL,Rsig,b,gsig)^2;
    M(7:9,k) = beta*M(7:9,k-1) + DJsmooth*(1-beta);
    U(7:9,k) = U(7:9,k-1) - stepsize(k-1-LL,Rstep,a,gstep)*sigma(k-1-LL,Rsig,b,gsig)^2*(M(7:9,k) + reggain(k-1-LL,Rreg,b,greg)*U(7:9,k-1));

    DJsmooth = 3*gamma*(J(3,k))*W(13:15)/sigma(k-1-LL,Rsig,b,gsig)^2;
    M(13:15,k) = beta*M(13:15,k-1) + DJsmooth*(1-beta);
    U(13:15,k) = U(13:15,k-1) - stepsize(k-1-LL,Rstep,a,gstep)*sigma(k-1-LL,Rsig,b,gsig)^2*(M(13:15,k) + reggain(k-1-LL,Rreg,b,greg)*U(13:15,k-1));
    Uprev1 = U(1:3,k-1);
    Uprev2 = U(7:9,k-1);
    Uprev3 = U(13:15,k-1);
