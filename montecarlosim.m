...
%     x3 = X(13:18,k-1); %s3
    ...
    U(1:3,k) = U(1:3,k-1) + alpha*(U(1:3,k-1)-Uprev1) - stepsize(k-1-LL,Rstep,a,gstep)*(M(1:3,k)) +0*stepsize(k-1-LL,Rstep,a,gstep)*resf1-0*stepsize(k-1-LL,Rstep,1.5,gstep)*(U(1:3,k-1));
    ...
    U(7:9,k) = U(7:9,k-1) + alpha*(U(7:9,k-1)-Uprev2) - stepsize(k-1-LL,Rstep,a,gstep)*(M(7:9,k)) + 0*stepsize(k-1-LL,Rstep,a,gstep)*resf2--0*stepsize(k-1-LL,Rstep,1.5,gstep)*(U(7:9,k-1));
    ...
    U(13:15,k) = U(13:15,k-1) + alpha*(U(13:15,k-1)-Uprev3) - stepsize(k-1-LL,Rstep,a,gstep)*(M(13:15,k)) +0*stepsize(k-1-LL,Rstep,a,gstep)*resf3-0*stepsize(k-1-LL,Rstep,1.5,gstep)*(U(13:15,k-1));
    Uprev1 = U(1:3,k-1);
    Uprev2 = U(7:9,k-1);
    Uprev3 = U(13:15,k-1);
    bound = bound + 0.001*(75-bound);
...
%     X(:,k) = q(end,:)';
    tau = -0.001;
    X(:,k) = exp(dt/tau)*X(:,k-1) + (1-exp(dt/tau))*(U(:,k)+alpha*(U(:,k)-U(:,k-1))+sigma(k-LL,Rsig,b,gsig)*W);
...
save('plotdataMC1000.mat','y12m','y13m','y23m','tspan','-v7.3');
